...
def print_game_data(toppings, game_time, completed_toppings):
  """
  Print the level data, and interface for the game screen.
  Avoides the use of repeated code.
  """
  move_to(2,35)
  print(Colors.BLUE + "Level:".center(len("Seconds Remaining:")) + Colors.END)
  move_to(3,35)
  print(Colors.WHITE + str(current_level).center(len("Seconds Remaining:")) + Colors.END)
...
def count_down(toppings, game_time):
  ...
  print_game_data(toppings, game_time, com)
  sleep(1)
...
def game_tacker(toppings, completed_toppings):
  """Updates the game stats when remaining_time has changed"""
  print("GAME TRACKER")
  stash_time = time_remaining
  while (True):
    print_game_data(toppings, time_remaining, completed_toppings)
    # if (stash_time < time_remaining):
...
def play_level(topping_count, total_time):
  ...
  time = threading.Thread(target=timer, args=(total_time, ))
  time.start()

  game_track = threading.Thread(target=game_tacker, args=(toppings, [], ))
  game_track.start()
